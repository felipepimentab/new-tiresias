...
/**
 * @brief	Set the sample rates for the encoder and the decoder, and the
 * bit rate for encoder.
 *
 * @note	If any of the values are 0, the corresponding configuration will
 * not be set.
 *
 * @param[in]	encoder_sample_rate_hz	Sample rate to be used by the encoder;
 * can be 0.
 * @param[in]	encoder_bitrate		Bit rate to be used by the encoder
 * (bps); can be 0.
 ...
 * @param[in]	bad_frame		Indication on missed or incomplete
 * frame.
 *
 * @return	0 on success, error otherwise.
 */
 ...
 *
 * @note	This can be used to reduce latency by adjusting the timing of
 * the completed frame that was sampled in relation to the connection interval
 * in Bluetooth LE.
 *
