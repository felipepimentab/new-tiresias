...
const static struct btn_config btn_cfg[] = {
#if DT_NODE_HAS_PROP(DT_ALIAS(sw0), gpios)
	{
		...
		.btn_spec = GPIO_DT_SPEC_GET(DT_ALIAS(sw0), gpios),
	},
#endif
};
/* clang-format on */
 ...
 */
static int pin_to_btn_idx(const struct device *port, uint8_t btn_pin, uint32_t *pin_idx)
{
	for (uint8_t i = 0; i < ARRAY_SIZE(btn_cfg); i++) {
		if (port == btn_cfg[i].btn_spec.port && btn_pin == btn_cfg[i].btn_spec.pin) {
			*pin_idx = i;
			return 0;
		}
	}
...
static void button_isr(const struct device *port, struct gpio_callback *cb, uint32_t pin_msk)
{
	...
	ret = pin_to_btn_idx(port, btn_pin, &btn_idx);
	ERR_CHK(ret);
...
int button_pressed(gpio_pin_t button_pin, bool *button_pressed)
{
	...
	for (uint8_t i = 0; i < ARRAY_SIZE(btn_cfg); i++) {
		if (button_pin != btn_cfg[i].btn_pin) {
			continue;
		}

		if (!gpio_is_ready_dt(&btn_cfg[i].btn_spec)) {
			return -ENODEV;
		}

		int ret = gpio_pin_get_dt(&btn_cfg[i].btn_spec);
		if (ret < 0) {
			return ret;
		}

		*button_pressed = ret == 1;
		return 0;
	}
...
int button_handler_init(void)
{
	...
	for (uint8_t i = 0; i < ARRAY_SIZE(btn_cfg); i++) {
		if (!gpio_is_ready_dt(&btn_cfg[i].btn_spec)) {
			LOG_ERR("Button GPIO controller not ready");
			return -ENODEV;
		}

		ret = gpio_pin_configure_dt(&btn_cfg[i].btn_spec, GPIO_INPUT);
		if (ret) {
			return ret;
		}

		gpio_init_callback(&btn_callback[i], button_isr, BIT(btn_cfg[i].btn_spec.pin));

		ret = gpio_add_callback(btn_cfg[i].btn_spec.port, &btn_callback[i]);
		if (ret) {
			return ret;
		}

		ret = gpio_pin_interrupt_configure_dt(&btn_cfg[i].btn_spec,
