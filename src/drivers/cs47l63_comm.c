...
static const struct spi_dt_spec spi
    = SPI_DT_SPEC_INST_GET(0, SPI_OP_MODE_MASTER | SPI_TRANSFER_MSB | SPI_WORD_SET(8) | SPI_LINES_SINGLE, 0);
...
static void cs47l63_comm_pin_int_handler(const struct device* gpio_port, struct gpio_callback* cb, uint32_t pins)
{
...
static uint32_t cs47l63_comm_reg_read(uint32_t bsp_dev_id, uint8_t* addr_buffer, uint32_t addr_length,
    uint8_t* data_buffer, uint32_t data_length, uint32_t pad_len)
{
  ...
  struct spi_buf rx_buf[] = { { .buf = addr_buffer, .len = addr_length }, { .buf = pad_buffer, .len = pad_len },
...
static uint32_t cs47l63_comm_reg_write(uint32_t bsp_dev_id, uint8_t* addr_buffer, uint32_t addr_length,
    uint8_t* data_buffer, uint32_t data_length, uint32_t pad_len)
{
  if (pad_len != PAD_LEN) {
    LOG_ERR("Trying to pad more than 4 bytes: %d", pad_len);
  ...
  struct spi_buf tx_buf[] = { { .buf = addr_buffer, .len = addr_length }, { .buf = pad_buffer, .len = pad_len },
    { .buf = data_buffer, .len = data_length } };
...
static uint32_t cs47l63_comm_i2c_read_repeated_start(uint32_t bsp_dev_id, uint8_t* write_buffer, uint32_t write_length,
    uint8_t* read_buffer, uint32_t read_length, bsp_callback_t cb, void* cb_arg)
{
  LOG_ERR("Tried to read repeated start I2C, not supported");
  return BSP_STATUS_FAIL;
}

static uint32_t cs47l63_comm_i2c_write(
    uint32_t bsp_dev_id, uint8_t* write_buffer, uint32_t write_length, bsp_callback_t cb, void* cb_arg)
{
  LOG_ERR("Tried writing to I2C, not supported");
  return BSP_STATUS_FAIL;
}

static uint32_t cs47l63_comm_i2c_db_write(uint32_t bsp_dev_id, uint8_t* write_buffer_0, uint32_t write_length_0,
    uint8_t* write_buffer_1, uint32_t write_length_1, bsp_callback_t cb, void* cb_arg)
{
...
int cs47l63_comm_init(cs47l63_t* cs47l63_driver)
{
  ...
  (void)k_thread_create(&cs47l63_data, cs47l63_stack, CONFIG_CS47L63_STACK_SIZE, (k_thread_entry_t)cs47l63_comm_thread,
      (void*)cs47l63_driver, NULL, NULL, K_PRIO_PREEMPT(CONFIG_CS47L63_THREAD_PRIO), 0,
  ...
  if (cs47l63_driver->devid != CS47L63_DEVID_VAL) {
    LOG_ERR("Wrong device id: 0x%02x, should be 0x%02x", cs47l63_driver->devid, CS47L63_DEVID_VAL);
    return -EIO;
  }
...
static bsp_driver_if_t bsp_driver_if_s = { .set_gpio = &cs47l63_comm_gpio_set,
  ...
  .i2c_read_repeated_start = &cs47l63_comm_i2c_read_repeated_start,
